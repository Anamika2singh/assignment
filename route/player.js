...
try{
 ...
 const matched= await v.check();
  ...
  if(!matched){
      res.status(422).json({statusCode:422,message:name+age+location+email_id+phone_number+password})
  }
else{

    let checkDuplicate=await useregister.findOne({phone_number:req.body.phone_number})
    if(checkDuplicate){
      res.status(400).json({
        'statusCode':400 ,
...
router.post('/login',async(req,res,next)=>{
    ...
    try{
        ...
        let found = await useregister.findOne({'phone_number':req.body.phone_number})
        // console.log(found.password);
            if(found == null){
               res.status(400).json({statusCode:400,message:"this phone_number not registered"})
            }
            else{
                bcrypt.compare(req.body.password,found.password,(err,user)=>{
                    if(user == true){
                        ...
                        check.token = token; 
                  res.status(200).json({statusCode:200,message:"login successfully" ,user_details:check})   
                    }
                    else{
...
router.post('/editProfile',verfiyToken,async(req,res,next)=>{
    ...
    if(get){
        ...
        console.log(arr);
       let update = await useregister.findByIdAndUpdate({_id:user._id},{$set:arr})
           console.log(update);
        let updatedData = await useregister.findOne({_id:user._id})
        console.log(updatedData);
       res.status(200).json({"statusCode":200,"message":"updated successfully","updated_data ": updatedData})
    }
   else{
...
router.get('/getgames',(req,res,next)=>{
    console.log("result");
    gameCreate.find({},{gameName:1}).then(games=>{
        res.status(200).json({
        'statusCode':200 ,
        "message":"list of games" ,result:games})}).catch(err=>{
...
router.get('/getresult',(req,res,next)=>{
    console.log("result");
    resulTable.find({},{ gameId:1,u1id:1,u2id:1,scoreu1:1,scoreu2:1,win:1,}).then(gamesres=>{
        res.status(200).json({
        'statusCode':200 ,
        "message":"result of games here" ,result:gamesres})}).catch(err=>{
            res.status(500).json({"statuscode":500,"message":"internal server error","error":err.message})
        } )
})
